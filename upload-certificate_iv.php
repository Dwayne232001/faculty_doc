...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update iv_organized set certificate ='NULL'  where id = $cardId";
             mysqli_query($conn,$query);
			 $success =1;
			$file_name = $cardId. "_iv.jpg";
			$file = $datapath."certificates/$file_name";
			if (!unlink($file)){
		...
		{
			$query = "Update iv_organized set certificate ='not_applicable'  where id = $cardId";
            mysqli_query($conn,$query);
			$success =1;
			$file_name = $cardId. "_iv.jpg";
			$file = $datapath."certificates/$file_name";
		...
		else if($_POST['applicable'] == 1)
		{
			...
			date_default_timezone_set('Asia/Kolkata');
			  $targetName=$datapath."certificates/".$_SESSION['F_NAME']."_iv"."_certificates_".date("d-m-Y H-i-s", time()).".$fileExt";  
			  if(empty($errors)==true){
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update iv_organized set certificate =' ".$targetName."'  where id = $cardId";
					 mysqli_query($conn,$query);
					$success =1;
