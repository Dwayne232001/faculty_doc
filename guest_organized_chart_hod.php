...
if(isset($_POST['submit'])){
  if(isset($_POST['name']) && $_POST['name']!="" && $_POST['name']!="All Faculty"){
      ...
      $query1 = "SELECT MAX(durationf) from guestlec WHERE Fac_ID=$fid";
  }
  else{
    $query1 = "SELECT MAX(durationf) from guestlec INNER JOIN facultydetails ON guestlec.Fac_ID=facultydetails.Fac_ID WHERE facultydetails.Dept='".$_SESSION['Dept']."'";
  }
...
while($startYear != $endYear) {
  ...
  if(isset($_POST['name']) && $_POST['name']!="" && $_POST['name']!="All Faculty"){
	  $query = "SELECT COUNT(*) FROM guestlec WHERE Fac_ID=$fid AND durationf >= '$date1' AND durationf <= '$date2'";
  }else{
    $query = "SELECT COUNT(*) FROM guestlec INNER JOIN facultydetails ON guestlec.Fac_ID=facultydetails.Fac_ID WHERE facultydetails.Dept='".$_SESSION['Dept']."' AND durationf >= '$date1' AND durationf <= '$date2'";
  }
...
while($startYear != $endYear) {
  ...
  $date2 = $temp_year.'-06-30';
  if(isset($_POST['name']) && $_POST['name']!="" && $_POST['name']!="All Faculty"){
    $query = "SELECT COUNT(*) FROM guestlec WHERE durationf >= '$date1' AND durationf <= '$date2' AND Fac_ID = $fid ";
  }else{
    $query = "SELECT COUNT(*) FROM guestlec INNER JOIN facultydetails ON guestlec.Fac_ID=facultydetails.Fac_ID WHERE facultydetails.Dept='".$_SESSION['Dept']."'AND durationf >= '$date1' AND durationf <= '$date2' ";
  }
	$result = mysqli_query($conn,$query);
        ...
        <section class="content">
		   ...
		   <div class="col-xs-12">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class="btn btn-primary" style="width:100px; height:40px" id="print" onclick="window.print();">Print Chart</button>
             <a class="btn btn-primary pull-right" style="height:40px" href="guest_organized_chart.php" >Select Another Faculty</a>
					</div>
				</div>
			</div>
		</div>
