...
$id = $_SESSION['id'];
$query = "SELECT * from co_curricular where co_curricular_ID ='" . $_SESSION['id'] . "' ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST['update'])) {
		...
		if (isset($_POST['applicable1'])) {
			if ($_POST['applicable1'] == 2) {
				...
				if (isset($_FILES['certificate'])) {
					...
					$fileExt = strtolower(end($temp));
					date_default_timezone_set('Asia/Kolkata');
					$targetName = $datapath . "certificates/" . $_SESSION['F_NAME'] . "_certificates" . date("d-m-Y H-i-s", time()) . "." . $fileExt;
					if (empty($errors) == true) {
						if (file_exists($targetName)) {
							unlink($targetName);
						}
		...
		if ($flag == 1) {
			...
			$sql = "UPDATE co_curricular set activity_name= $activityname, 
									Date_from=$startdate,
									Date_to=$endDate, 
									...
									event = '$event',
									level = '$status_act'
									WHERE co_curricular_ID = '" . $_SESSION['id'] . "' ";
			...
			<div class="col-md-8">
				...
				<div class="box box-primary">
					...
					<form role="form" method="POST" class="row" action="" style="margin:10px;" enctype="multipart/form-data">
						...
						<div class="form-group col-md-6">
							<label for="activity-name">Title of Project/Thesis/Award/Recognition *</label>
							<input required type="text" class="form-control input-lg" name="activityname" value='<?php echo $activityname; ?>'>
						</div><br><br>

						<div class="form-group col-md-6">
							<label for="event">Event *</label>
							<input required type="text" class="form-control input-lg" name="event" value='<?php echo $event; ?>'>
						</div><br><br>
						...
						<div class="form-group col-md-6">
							...
							<input required <?php echo "value = $startDate"; ?> type="date" class="form-control input-lg" id="start-date" name="startdate" placeholder="03:10:10">
						</div><br><br>

						<div class="form-group col-md-6">
							<label for="end-date">End Date *</label>
							<input required <?php echo "value = $endDate"; ?> type="date" class="form-control input-lg" id="end-date" name="enddate" placeholder="03:10:10">
						</div><br><br>
						...
						<div class="form-group col-md-6">
							<label for="purpose-of-activity">Details of Activity *</label>
						...
						<div class="form-group col-md-12">
							<?php
							if ($_SESSION['type'] == 'hod') {
								echo '<a href="2_dashboard_hod_cocurricular.php" type="button" class="btn btn-warning btn-lg">Cancel</a>';
							} else {
								echo '<a href="2_dashboard_cocurricular.php" type="button" class="btn btn-warning btn-lg">Cancel</a>';
							}
							?>
