...
$query = "SELECT * FROM iv_organized WHERE id=$cardId";
...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update iv_organized set attendance ='NULL'  where id = $cardId";
             mysqli_query($conn,$query);
			 $success =1;
			$file_name = $cardId. "_iv.jpg";
			$file = $datapath."attendance/$file_name";
			if (!unlink($file)){
		...
		{
			$query = "Update iv_organized set attendance ='not_applicable'  where id = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			$file_name = $cardId. "_iv.jpg";
			$file = $datapath."attendance/$file_name";
		...
		else if($_POST['applicable'] == 1)
		{
			...
			date_default_timezone_set('Asia/Kolkata');
			  $targetName=$datapath."attendance/".$_SESSION['F_NAME']."_iv"."_attendance_".date("d-m-Y H-i-s", time()).".$fileExt";  
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update iv_organized set attendance =' ".$targetName."'  where id = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
...
if($success == 1)
{
	 if($_SESSION['type'] == 'hod' || $_SESSION['type'] == 'cod' || $_SESSION['type'] == 'com')
						{
						   header("location:2_dashboard_iv_hod.php?alert=update");

						}
						else
						{
							header("location:2_dashboard_iv.php?alert=update");
              ...
              <div class="box box-primary">
				...
				<div class="box-header with-border">
                  <i style="font-size:20px" class="fa fa-edit"></i>
					<h3 class="box-title"><b>Upload Attendance Record</b></h3>
                </div>                </div><!-- /.box-header -->
                <!-- form start -->
...
if($_SESSION['type'] == 'hod' || $_SESSION['type'] == 'cod' || $_SESSION['type'] == 'com')
{ ?>
        <a href="2_dashboard_iv_hod.php" type="button" class="btn btn-warning btn-lg">Cancel</a>
<?php
}
else
{
?>
      <a href="2_dashboard_iv.php" type="button" class="btn btn-warning btn-lg">Cancel</a>
<?php
}
?>			 
