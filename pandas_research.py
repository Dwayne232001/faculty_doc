...
try:
    expected_headers = ['Sr. No.','Name of Faculty/Staff','Research Title','Start Date (DD-MM-YYYY)','End Date (DD-MM-YYYY)','Submitted to','Principle Investigator','Co-Investigator','Proposed Amount','Approved?','Amount Sanctioned','Awards Won','Tenure Of Project (in Months)']
...
for i in range(0, df.shape[0]):
    ...
    try:
        details = '%'+faclist[2].strip().strip('"').strip('.')+'%'
        # organizer = '%'+faclist[3].strip().strip('"').strip('.')+'%'
        q_check = "SELECT 1 from researchdetails where Fac_id=" + str(faclist[0])+" AND researchTitle LIKE '"+str(details)+"'"
        # print(q_check)
        ...
        if result == 0:
            ...
            # print(faclist)
            print(val)
            sql = "INSERT INTO researchdetails(Fac_id,facultyName,researchTitle,fromDate,toDate,submittedTo,principleInvestigator,coInvestigator,proposedAmount,radioApproval,amountSanctioned,awardsWon,tenure,noofdays,month,year) VALUES ('%s', '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % val
            # print(sql)
