...
try:
    expected_headers = ['Sr. No.','Name of Faculty/Staff','Title of Project/Thesis/Award/Recognition ','Start Date (DD-MM-YYYY)','End Date (DD-MM-YYYY)','Organized By','Purpose of Activity']
...
for i in range(0, df.shape[0]):
    ...
    try:
        ...
        q_check = "SELECT 1 from co_curricular where Fac_id=" + str(faclist[0])+" AND activity_name LIKE '"+str(name)+"'"
        # print(q_check)
        ...
        if result == 0:
            ...
            # print(faclist)
            print(val)
            sql = "INSERT INTO co_curricular(Fac_id,activity_name,Date_from,Date_to,organized_by,purpose_of_activity,noofdays,month,year) VALUES ('%s', '%s','%s','%s','%s','%s','%s','%s','%s')" % val
            # print(sql)
