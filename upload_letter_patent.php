...
$query = "SELECT * FROM patents WHERE patent_id=$cardId";
...
if(isset($_POST['insert-letter'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update patents set letter_path ='NULL'  where patent_id = $cardId";
		...
		{
			$query = "Update patents set letter_path ='not_applicable'  where patent_id = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
		...
		else if($_POST['applicable'] == 1)
		{
			if(isset($_FILES['letter']))
			{
			  ...
			  $fileTmp = $_FILES['letter']['tmp_name'];
			  $fileType = $_FILES['letter']['type'];
			  $fileExt=strtolower(end(explode('.',$_FILES['letter']['name'])));
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update patents set letter_path =' ".$targetName."'  where patent_id = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
...
if($success == 1)
{
	 if($_SESSION['type'] == 'hod' || $_SESSION['type'] == 'cod' || $_SESSION['type'] == 'com')
						...
						{
								header("location:2_dashboard_patent.php");
                ...
                <form action="" method="POST" enctype="multipart/form-data" class="row">
					...
					<div class='second-reveal'>
					 <div class="form-group col-md-6">
                         ...
                         <label for="card-letter">Certificate * </label>
                         <input  type="file"   class="form-control input-lg" id="card-letter" name="letter">
                    </div> 
					</div>
...
{
?>
      <a href="2_dashboard_patent.php" type="button" class="btn btn-warning btn-lg">Cancel</a>
<?php
}
?>			 
                         <div class="pull-right"> 
						 
                             <button name="insert-letter" type="submit" class="btn btn-success  btn-lg">Insert</button>
                         </div>
                    </div> 
                 </form>
