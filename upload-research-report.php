...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update researchdetails set reportPath ='NULL'  where researchId = $cardId";
		...
		{
			$query = "Update researchdetails set reportPath ='not_applicable'  where researchId = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
		...
		else if($_POST['applicable'] == 1)
		{
			if(isset($_FILES['image']))
			{
			  ...
			  $targetName=$datapath."reports/".$_POST['id']."_reports_".date("d-m-Y H-i-s", time()).".".$fileExt;  
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update researchdetails set reportPath =' ".$targetName."'  where researchId = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
