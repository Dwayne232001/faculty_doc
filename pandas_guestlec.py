...
try:
    expected_headers = ['Sr. No.','Name of Faculty/Staff','Topic','Start Date (DD-MM-YYYY)','End Date (DD-MM-YYYY)','Resource Person Name','Designation','Organisation','Target Audience']
...
for i in range(0, df.shape[0]):
    ...
    try:
        details = '%'+faclist[1].strip().strip('"').strip('.')+'%'
        # organizer = '%'+faclist[3].strip().strip('"').strip('.')+'%'
        q_check = "SELECT 1 from guestlec where fac_id=" + str(faclist[0])+" AND topic LIKE '"+str(details)+"'"
        # print(q_check)
        ...
        if result == 0:
            ...
            # print(faclist)
            print(val)
            sql = "INSERT INTO guestlec(fac_id,topic,durationf,durationt,name,designation,organisation,targetaudience,noofdays,month,year) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % val
            # print(sql)
