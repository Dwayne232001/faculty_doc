...
for i in range(0, df.shape[0]):
    ...
    try:
        ...
        q_check = "SELECT 1 from ex_curricular where Fac_id=" + str(faclist[0])+" AND activity_name LIKE '"+str(name)+"'"
        # print(q_check)
        ...
        if result == 0:
            ...
            # print(faclist)
            print(val)
            sql = "INSERT INTO ex_curricular(Fac_id,activity_name,Date_from,Date_to,organized_by,purpose_of_activity,noofdays) VALUES ('%s', '%s','%s','%s','%s','%s','%s')" % val
            # print(sql)
