...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update iv_organized set permission ='NULL'  where id = $cardId";
             mysqli_query($conn,$query);
			 $success =1;
			$file_name = $cardId. "_iv.jpg";
			$file = $datapath."permissions/$file_name";
			if (!unlink($file)){
		...
		{
			$query = "Update iv_organized set permission ='not_applicable'  where id = $cardId";
            mysqli_query($conn,$query);
			$success =1;
			$file_name = $cardId. "_iv.jpg";
			$file = $datapath."permissions/$file_name";
		...
		else if($_POST['applicable'] == 1)
		{
			...
			date_default_timezone_set('Asia/Kolkata');
			  $targetName=$datapath."permissions/".$_SESSION['F_NAME']."_iv"."_permissions_".date("d-m-Y H-i-s", time()).".$fileExt";  
			  if(empty($errors)==true){
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update iv_organized set permission =' ".$targetName."'  where id = $cardId";
					 mysqli_query($conn,$query);
					$success =1;
              ...
              <div class="box box-primary">
				...
				<div class="box-header with-border">
                  <i style="font-size:20px" class="fa fa-edit"></i>
					<h3 class="box-title"><b>Upload Permission</b></h3>
                </div>                </div><!-- /.box-header -->
                <!-- form start -->
